...
#################################################################
#								#
# Make the configuration file for adding new pairs to existing  #
# SLC stacks for isce2 stack processor. 			#
#								#
#			Lin, Li-Chieh				#
...
if ($#argv != 1) then
  echo ""
  echo "Make the configuation file for adding new pairs to existing SLC stacks for isce2 stack processor"
  echo "Current version only supports topsStack processor (stackSentinel.py)"
  echo ""
  echo "Example:"
  echo "  csh pop_stack_config.csh sentinel > config.txt"
  echo ""
  exit 1
endif
